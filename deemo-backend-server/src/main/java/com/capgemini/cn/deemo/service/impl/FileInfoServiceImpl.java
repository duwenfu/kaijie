...
public class FileInfoServiceImpl implements FileInfoService {
    ...
    public Integer restoreFilesFromTrash(DeleteVo deleteVo) {
        ...
        for (Long fileId : deleteVo.getIds()) {
            ...
            // 如果父文件夹是删除状态, 恢复父文件夹, 循环直到上一层目录没有被删除或者为根目录为止
            FileInfo fileInfo = fileInfoMapper.getFile(fileId);
            FileInfo parent = fileInfoMapper.getFile(fileInfo.getParentId());
            while (parent.getIsDeleted() && parent.getFileId() == 1L) {
                fileInfoMapper.restoreFileFromTrash(parent.getFileId());
                parent = fileInfoMapper.getFile(parent.getParentId());
            }
        }
